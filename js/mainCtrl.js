angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  $scope.gridOptions = {
      ...
      data: 'songData',
      height: '110px',
      sortInfo: {fields: ['Song', 'Artist', 'Album', 'Genre','Release'], directions: ['asc']},
      columnDefs: [
        {field: 'Play', displayName: 'Play', width: '40px', cellTemplate: '<div class="ngCellText" ng-class="col.colIndex()"><a href="{{row.getProperty(col.field)}}"><img src="http://www.icty.org/x/image/Miscellaneous/play_icon30x30.png"></a></div>'},
        {field: 'Artist', displayName: 'Artist'},
        {field: 'Song', displayName: 'Song Title'},
        {field: 'Album', displayName: 'Album'},
        {field: 'AlbumArt', displayName: 'Artwork', width: '110px', cellTemplate: '<div class="ngCellText" ng-class="col.colIndex()"><img src="{{row.getProperty(col.field)}}"></div>'},
        {field: 'Genre', displayName: 'Genre'},
        {field: 'Cost', displayName: 'Price'},
        {field: 'Release', displayName: 'Release Date', cellFilter: 'date:\'MMM yyyy\''}
      ]
  };
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getSongData = function (artist){
      itunesService.getSongData(artist).then(function(dataFromService){
